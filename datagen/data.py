...
class MtcnnDataset(object):
    ...
    def __init__(self, output_folder, net_stage, batch_size, suffix,num_workers = 1):
        """
        ...
        if net_stage == 'pnet':
            ...
            raise AttributeError(
                "Parameter 'net_stage' must be one of 'pnet', 'rnet'  instead of %s." % net_stage)
    ...
    def get_iter(self):
        pos_loader = DataLoader(self.pos, self.pos_batch, shuffle=True,num_workers = self.num_workers)
        part_loader = DataLoader(self.part, self.part_batch, shuffle=True,num_workers = self.num_workers)
        neg_loader = DataLoader(self.neg, self.neg_batch, shuffle=True,num_workers = self.num_workers)
