        ...
        with open(meta_file) as f:
            ...
            for line in f:
                if flag == 0:
                    ...
                    num_bb = int(line.strip())
                    current_dict['num_bb'] = num_bb
                    current_dict['meta_data'] = []
                    flag = 2
                elif flag == 2:
                    values = line.strip().split(' ')
                    if len(values) >= 10:  # Ensure there are at least 10 values
                        cur = [int(i) for i in values]
                        if cur[7] == 0:
                            current_dict['meta_data'].append(cur)
                        else:
                            num_bb -= 1
                        if len(current_dict['meta_data']) == num_bb:
